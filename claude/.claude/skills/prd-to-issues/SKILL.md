...
Ask the user for the PRD Linear issue identifier (e.g. ENG-123) or URL.

If the PRD is not already in your context window, fetch it with the `mcp__linear__get_issue` tool.
...
For each approved slice, create a Linear issue using the `mcp__linear__save_issue` tool. Use the issue body template below.
...
<prd-issue-identifier> (e.g. ENG-123)
